...
class RealmService {
    ...
    func saveOrUpdateWeather(_ weatherModel: WeatherRealmModel) throws {
        do {
            let realm = try Realm()
            try realm.write {
                if let existingWeatherModel = realm.objects(WeatherRealmModel.self).first {
                    ...
                    print("Updated data: \(existingWeatherModel)")
                } else {
                    save(weatherModel, in: realm)
                    print("Realm: Saved successfully")
                    print("Saved data: \(weatherModel)")
                }
            }
        } catch {
    ...
    func fetchCachedWeather() -> WeatherRealmModel? {
